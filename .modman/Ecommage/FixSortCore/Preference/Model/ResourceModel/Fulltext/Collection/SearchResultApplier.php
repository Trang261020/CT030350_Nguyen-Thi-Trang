<?php
namespace Ecommage\FixSortCore\Preference\Model\ResourceModel\Fulltext\Collection;
...
class SearchResultApplier extends \Magento\Elasticsearch\Model\ResourceModel\Fulltext\Collection\SearchResultApplier
{
    ...
    private function getProductIdsBySaleability(): array
    {
        ...
        if (!$this->hasShowOutOfStockStatus() || array_key_exists('brands',$request))
        {
            return $ids;
        }
        ...
        foreach ($sortOrders as $field => $dir) {
            if ($field === 'name') {
                ...
                $query->joinLeft(
                    ...
                    . $storeId . ')',
                    ['price_index.min_price AS price']
                );
            }
            $columnFilters = [];
