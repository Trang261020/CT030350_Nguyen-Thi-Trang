        ...
        return function (Shipping) {
            return Shipping.extend({
                ...
                validateShipping: function () {
                    ...
                    var self = this;
                    var isHasRegion = self.getShippingAddress();
                ...
                /**
                 * get Shipping address from checkoutConfig
                 */
                getShippingAddress: function(){
                    var customerData = window.checkoutConfig.customerData;
                    var defaultShipping = customerData.default_shipping ? customerData.default_shipping : '';
                    var customerAddress = customerData.addresses ? customerData.addresses : '';
                    var isHasDefaultShipping = 0;

                    if(defaultShipping){
                        $.each(customerAddress, function (index,value){
                            if(index == defaultShipping && value.region_id){
                                isHasDefaultShipping = 1;
                            }
                        });
                    }
                    return isHasDefaultShipping;
                },
