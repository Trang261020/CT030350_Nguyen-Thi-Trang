define([
...
], function (Component, $, quote,priceUtil) {
    'use strict';
    ...
    return Component.extend({
        ...
        getDiscountDataFromTotals: function (totals) {
            var priceConfigPrecision = window.checkoutConfig.priceFormat ? window.checkoutConfig.priceFormat.precision : 0;
            var pattern = window.checkoutConfig.priceFormat.pattern ? window.checkoutConfig.priceFormat.pattern : '₫';
            pattern = pattern.replace('%s','');
            var globalPriceFormat = {
                requiredPrecision: priceConfigPrecision // get config
            };

            if (totals.extension_attributes && totals.extension_attributes.amrule_discount_breakdown) {
                var promo = totals.extension_attributes.amrule_discount_breakdown;
                $.each(promo, function(index,val){
                    if(val.rule_amount){
                        var price = parseFloat(val.rule_amount.replace(pattern,'').replaceAll('.',''));
                        val.rule_amount = priceUtil.formatPrice(price,globalPriceFormat);
                    }
                })
